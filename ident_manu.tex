...
\author{
  ...
  \vspace{1in} 
  \\ Version Date:   Thu Jul 7 15:16:18 2016 -0500
	}
\date{}
\maketitle
\end{singlespace}
...
Local sensitivity analyses showed that \ac{do} was sensitive to perturbations in 140 of 250 parameters (56\% of total) in FishTank. Within each parameter category, \ac{do} was sensitive to four parameters for optics (36\% of all optic parameters, \cref{tab:optsens}), seven for organic matter (24\%, \cref{tab:omsens}), 103 for phytoplankton (66\%, \cref{tab:phytosens}), seven for temparature (22\%, \cref{tab:tempsens}), and 19 for zooplankton (86\%, \cref{tab:zoopsens}). Although \ac{do} had the greatest sensitivity to parameters in the zooplankton category (as percentage of total), the relative effects varied. Among all parameters, sensitivity values ranged from $3.18\times 10^{-6}$ for \textit{Kcdom} (optics) to $328.35$ for \textit{Qc$_{p1}$} (phytoplankton).

what parameters in each category had the greatest sensitivity (\cref{fig:sensplo}), what were sensitivity magnitudes between categories, did these vary by phyto or zoop groups?  

Plotting the raw values from the sensitivity analysis provides a visual assessment of changes (\cref{fig:sensplo}).
