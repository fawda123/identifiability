...
The identifiability analyses suggested that most parameter subsets exceeded the threshold of $\gamma = 10, 15$, providing further justification for using selection heuristics for parameter optimization.  Parameter identifiability (as $\gamma$) increased at different rates depending on the parameter category or the number of top parameters that were selected (\cref{fig:identbox}).  By category, identifiability was lowest for parameter subsets in the phytoplankton (\Sexpr{collcatpers$Phytoplankton[1]}\% less than $\gamma = 15$, \Sexpr{collcatpers$Phytoplankton[2]}\% less than $\gamma = 10$)  and zooplankton categories (\Sexpr{collcatpers$Zooplankton[1]}\%, \Sexpr{collcatpers$Zooplankton[2]}\%), whereas a majority of combinations for temperature were identifiable (\Sexpr{collcatpers$Temperature[1]}\%, \Sexpr{collcatpers$Temperature[2]}\%).  All subset combinations for optics and organic matter parmeters had $\gamma < 15$.  All parameter subsets for choosing the top, top two, and top three parameters in each category were identifiable (\cref{fig:identbox}), whereas a majority were identifiable for choosing the top four (\Sexpr{colltoppers$top4[1]}\% less than $\gamma = 15$, \Sexpr{colltoppers$top4[2]}\% less than $\gamma = 10$) and top five (\Sexpr{colltoppers$top4[1]}\%, \Sexpr{colltoppers$top4[2]}\%) parameters.  A comparison of average and median identifiability by parameter category or top parameters in each category... something about jump
