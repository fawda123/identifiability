...
foot.val <- '\\footnotesize *Starting \\ac{rmse} $= 72.140$'
...
\caption{Conceptual representation of biogeochemical components modelled in \ac{cgem} (complete equations in \citealt{Eldridge10}). FishTank implements a \ac{zerod} version of the model that does not include any advection, mixing, or sediment diagenesis.}
\label{fig:mod_schem}
\end{figure}
