...
\begin{comment}
\begin{enumerate}
\item Hypoxia formation and use of simulation/biogeochemical/process-based models overview, contrast with statistical models
...
Formation of hypoxia in bottom waters of coastal oceans occurs primarily as a result of excess nutrient inputs from land-based sources.  These events are detrimental to aquatic organisms and have significant negative effects on economic resources derived from coastal ecosystems.  An understanding of the biological, physical, and chemical processes that contribute to the growth of hypoxic areas is a critical concern for mitigating and preventing these negative impacts.  Numerical ecosystem models have been important tools for describing current knowledge of ecosystem processes that contribute to hypoxia formation and for predicting the effects of proposed management activities or future scenarios.  Unlike statistical models with more generic structures, simulation and process-based models include explicit descriptions of relevant processes that are constrained by empirical or observational data relevant to the system of interest.  These models are often coupled with hydrodynamic grids to provide spatially-explicit representations of patterns in three dimensions. Coupled hydrodynamic and bio-geo-chemical models have been developed specifically to describe hypoxic conditions on the \ac{lcs} in the northern \ac{gom}
...
<<sensalltile, echo = F, fig.height = 9, fig.width = 9, out.width = '\\textwidth', fig.cap = 'Sensitivity values (L1, \\cref{l1}) for local analyses of all state variables.  Values are truncated by the 5\\textsuperscript{th} and 95\\textsuperscript{th} sensitivity values between all parameters for visual appearance. Parameters are grouped by category (plot strips).  Phytoplankton and zooplankton categories are further separated into one through six, and one and two groups (p1-p6, z1, z2), respectively.'>>=
data(sens_ests_cat_all)

tmp <- reshape2::melt(sens_ests_cat_all, id.var = c('Parameter', 'error', 'Category'))

lims <- with(tmp, quantile(error, c(0.05, 0.95))) %>% 
  as.numeric

stts_lev <- rev(c('NH4', 'Chla_mg_tot', 'O2', 'irradiance', 'NO3', 'OM1', 'OM2', 'PO4'))
stts_lab <- rev(c('Ammonium', 'Chlorophyll', 'Dissolved Oxygen', 'Irradiance', 'Nitrate', 'OM1', 'OM2', 'Phosphate'))

toplo <- filter(tmp, error < lims[2] & error > lims[1]) %>% 
  mutate(
    L1 = factor(L1, levels = stts_lev, labels = stts_lab)
  ) %>% 
  rename(Sensitivity = error)

toplo1 <- filter(toplo, !Category %in% c('Phytoplankton', 'Zooplankton')) %>% 
  mutate(Parameter = as.character(par_txt(Parameter, frm = 'exp')))
toplo2 <- filter(toplo, Category %in% c('Phytoplankton')) %>% 
  mutate(
    grp = gsub('^.*_([1-6])$', 'p\\1', Parameter), 
    Parameter = as.character(par_txt(Parameter, frm = 'exp'))
  )
toplo3 <- filter(toplo, Category %in% c('Zooplankton')) %>% 
  mutate(
    grp = gsub('^.*_([1-2])$', 'z\\1', Parameter), 
    Parameter = as.character(par_txt(Parameter, frm = 'exp'))
  )
...
p1 <- ggplot(toplo1, aes(x = Parameter, y = L1, fill = Sensitivity)) + 
  geom_tile() + 
  pbase +
...
p2 <- ggplot(toplo2, aes(x = Parameter, y = L1, fill = Sensitivity)) + 
  geom_tile() + 
  pbase +
  scale_x_discrete(expand = c(0, 0), 
    breaks = toplo2$Parameter, 
    labels = parse(text = toplo2$Parameter)
    ) + 
  scale_y_discrete(expand = c(0, 0)) +
  scale_fill_distiller(palette = "Spectral", limits = lims) + 
  facet_grid(~grp, scales = 'free') + 
  ggtitle('Phytoplankton')
p3 <- ggplot(toplo3, aes(x = Parameter, y = L1, fill = Sensitivity)) + 
  geom_tile() + 
  pbase +
  scale_x_discrete(expand = c(0, 0), 
    breaks = toplo3$Parameter, 
    labels = parse(text = toplo3$Parameter)
    ) + 
  scale_y_discrete(expand = c(0, 0)) + 
  scale_fill_distiller(palette = "Spectral", limits = lims) + 
  facet_grid(~grp, scales = 'free') + 
...
grid.arrange(
  pleg, p1, p2, p3, ncol = 1, 
  heights = c(0.2, 1, 0.85, 1),
  bottom = 'Parameter', 
  left = 'State variable'
)
@
