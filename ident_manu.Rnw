...
p1 <- ggplot(toplo, aes(x = N, colour = Category, group = N)) + 
...
p2 <- ggplot(toplo2, aes(x = N, colour = npercat, group = N)) + 
  geom_point(aes(y = ave, shape = 'Ave')) + 
  geom_point(aes(y = med, shape = 'Med')) + 
  geom_errorbar(aes(ymin = min, ymax = max), width = 0) + 
  facet_wrap(~ npercat, ncol = 1, scales = 'free_y') +
  theme_minimal() + 
  scale_colour_brewer(palette = colset, guide = F) + 
  theme(
    axis.line.x = element_line(), 
    axis.line.y = element_line(), 
    axis.title.x = element_blank(), 
