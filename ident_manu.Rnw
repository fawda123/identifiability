...
The core unit of \ac{cgem} is FishTank, a \ac{zerod} model that implements the biogeochemical equations in \citet{Eldridge10} and does not include any form of physical transport (i.e., advection, mixing, or surface flux).  Although FishTank was developed for specific application in \ac{cgem}, it can easily be applied to other hydrodynamic grids. Accordingly, the sensitivity and identifiability analyses described below are informative for both the \ac{lcs} gridded model as well as potential applications to different systems.  The FishTank model provides estimates for the 36 state variables described above using a \ac{zerod} parcel that is uniformly mixed as a closed system.  A set of initial conditions is provided on execution of the model that is based on observations of relevant variables obtained from research cruises in the northern \ac{gom} during April, June, and September of 2006 \citep[Table 1 in][]{Murrell14}. 
...
<<sensalltile, eval = T, echo = F, fig.height = 6, fig.width = 3.5, out.width = '0.7\\textwidth', fig.cap = 'Sensitivity values (L1, \\cref{l1}) for local analyses of all state variables. Parameters are grouped by category: optics, organic matter, phytoplankton, zooplankton, temperature, and zoplankton.  See \\cref{tab:dosens} for L1 values for \\ac{do} and \\cref{tab:nh4sens,tab:chlsens,tab:irrsens,tab:no3sens,tab:om1sens,tab:om2sens,tab:po4sens} for the other state variables.'>>=
...
swtch <- data.frame(
  lab = c('Temperature', 'Uptake', 'Quota', 'Chla:C', 'Photosynthesis', 'Growth'),
  sim = c('sigmoidal', 'Michaelis-Menten', 'Droop', 'regression', 'photoinhibition', 'minimum'),
  com = c('Arrenhius', 'Geider', 'Flynn', 'Cloern', 'nutrient dependent', '\\textit{umax} is nutrient dependent'), 
  stringsAsFactors = F
  ) %>% 
  rename(
    `Switch type` = lab, 
...
cap_val <- 'Model switches that were used to evaluate effects of structural uncertainty on parameter sensitivity.'
...
<<inistreval, fig.cap = 'Effects of initial and structural conditions on parameter sensitivity.', fig.height = 7, fig.width = 8, out.width = '\\textwidth', eval = T, echo = F, warning = F, message = F>>=
# initial conditions
data(ini_eval)
data(struct_eval)
