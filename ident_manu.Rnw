...
tmpest <- sens_ests_all[['O2']]
tmpest_cat <- sens_ests_cat_all[['O2']]
...
totab <- tmpest_cat %>% 
  select(Parameter) %>% 
  left_join(., tmpest$sens, by = 'Parameter') %>% 
  ...
  mutate(
    ...
    L1 = sapply(L1, scinot, digits = 2, pow = 2)
  ) %>% 
  rename(
    Description = lngs, 
    Value = vals
...
cap.val<- paste('Sensitivities of \\ac{do} to perturbation of indivividual parameters.  Sensitivities are based on a 50\\% increase from the initial parameter value, where $L1$ summarizes differences in model output from the default (see \\cref{l1}).  Parameters that did not affect \\ac{do} are not shown.  Parameters are grouped by categories as optics, temperature, phytoplankton, zooplankton, and organic matter.')
...
labs <- parcats2(as_df = T) %>% 
    rename(Parameter = shrt) %>% 
    select(Parameter, lngs)

# format tocal_all for identifiability table
# add long parameter name
# get parameter combos with coll < 15 + 1
# format error and coll with scinot
# format category label so no duplicates
totab <- tocal_all$top_all %>% 
  ...
  mutate(
    ...
    Rank = paste0(Rank, '\\textsuperscript{', Category, '}')
    ) %>%
  group_by(stts) %>% 
  nest %>% 
  unnest %>% 
  select(stts, lngs, Parameter, Rank, error, coll) %>% 
  rename(
    Description = lngs, 
    L1 = error
  ) %>% 
  mutate(
    ...
    Description = paste0('\\footnotesize{', Description, '}'),
    Parameter = paste0('\\footnotesize{', Parameter, '}')
    ) %>% 
  arrange(stts)
    
identall1 <- filter(totab, stts %in% 'Dissolved Oxygen')

# final table formatting
descrip <- as.character(identall1$Description)
identall1 <- identall1[,-c(1, 2)]
cap_val<- 'Parameter identifiability for dissolved oxygen.  Selection followed the second heuristic where parameters were selected by decreasing sensitivity up to maximum $\\gamma < 15$.  Rank describes the parameter sensitivity in each category (O: optics, OM: organic matter, P: phytoplankton, T: temperature, Z: zooplankton).'
colnms <- c('Parameter', 'Rank', 'L1', '$\\gamma$')

latex( 
  identall1,
  file = '',
  rowlabel = 'Parameter description',
  caption = cap_val,
  caption.loc = 'top',
  size = 'normalsize',
  label = 'tab:identall1',
...
totab <- lapply(tocal_all, function(x) x$top) %>% 
  ...
  mutate(
    stts = as.character(stts),
    Description = paste0('\\scriptsize{', Description, '}'),
    Parameter = paste0('\\scriptsize{', Parameter, '}')
    ) %>% 
  arrange(stts)
    
identall2 <- filter(totab, !stts %in% 'Dissolved Oxygen')

# final table formatting
stts <- identall2$stts
descrip <- as.character(identall2$Description)
identall2 <- identall2[,-c(1, 2)]
cap_val<- 'Parameter identifiability for ammonium, chlorophyll, irradiance, nitrate, OM1, OM2, and phosphate.  Selection followed the second heuristic where parameters were selected by decreasing sensitivity up to maximum $\\gamma < 15$.  Rank describes the parameter sensitivity in each category (O: optics, OM: organic matter, P: phytoplankton, T: temperature, Z: zooplankton).'
colnms <- c('Parameter', 'Rank', 'L1', '$\\gamma$')

latex( 
  identall2,
  file = '',
  rowlabel = 'State variable, parameter description',
  caption = cap_val,
  caption.loc = 'top',
  size = 'footnotesize',
  label = 'tab:identall2',
  rgroup = unique(as.character(stts)),
  n.rgroup = as.numeric(table(as.character(stts))),
  rowname = descrip,
  colheads = colnms,
  rgroupTexCmd = NULL
  )
...
<<excltemp, eval = F, fig.cap = 'Identifiability (as $\\gamma$, \\cref{gameq}) of temperature parameters for subset combinations in \\cref{fig:identbox}.  Identifiability is evaluated for subsets that excluded and included the parameters at the top of each plot. Identifiability of including all seven parameters is in \\cref{fig:identbox}. Grey lines indicate potential thresholds at $\\gamma = 10, 15$ for maximum acceptable identifiability.', fig.height = 8, fig.width = 5, echo = F, cache = TRUE, out.width = "0.6\\textwidth">>=
data(cat_ident)
...
<<heurist_stts, eval = F, fig.cap = 'Identifiability (as $\\gamma$, \\cref{gameq}) of selecting parameters for selected state variables. Parameters are selected by decreasing sensitivity indepent of parameter categories. Grey lines indicate potential thresholds at $\\gamma = 10, 15$ for maximum acceptable identifiability. Selection stops after $\\gamma > 15$.  See \\cref{tab:identall2} for values.', fig.height = 5, fig.width = 9, echo = F, cache = TRUE, out.width = '\\textwidth'>>=
data(tocal_all)

# factor levels
cats <- c('Optics', 'Organic Matter', 'Phytoplankton', 'Temperature', 'Zooplankton')
stts_lev <- c('NH4', 'Chla_mg_tot', 'irradiance', 'NO3', 'OM1', 'OM2', 'PO4')
stts_lab <- c('Ammonium', 'Chlorophyll', 'Irradiance', 'Nitrate', 'OM1', 'OM2', 'Phosphate')
...
<<sensalltile, eval = T, echo = F, fig.height = 7, fig.width = 6, out.width = '\\textwidth', fig.cap = 'Sensitivity values (L1, \\cref{l1}) for local analyses of all state variables. Parameters are grouped by category: optics, organic matter, phytoplankton, zooplankton, temperature, and zoplankton.  See \\cref{tab:dosens} for L1 values for \\ac{do}.'>>=
data(sens_ests_cat_all)

tmp <- reshape2::melt(sens_ests_cat_all, id.var = c('Parameter', 'error', 'Category'))

lims <- with(tmp, quantile(error)) %>% 
  as.numeric
...
toplo1 <- mutate(toplo, Parameter = as.character(par_txt(Parameter, frm = 'exp')))
...
ggplot(toplo1, aes(y = Parameter, x = L1, fill = Sensitivity)) + 
  geom_tile(colour = 'black') + 
  pbase +
  scale_y_discrete('Parameter', 
    expand = c(0, 0),
    breaks = toplo1$Parameter, 
    labels = parse(text = toplo1$Parameter)
    ) + 
