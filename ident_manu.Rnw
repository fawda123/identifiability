...
<<catident, fig.cap = 'Identifiability (as $\\gamma$, \\cref{gameq}) of selecting parameters by category (optics, organic matter, phytoplankton, temperature, zooplankton).  Parameters are selected in each category by decreasing sensitivity (\\cref{tab:optsens,tab:tempsens,tab:phytosens,tab:zoopsens,tab:omsens}). Grey lines indicate thresholds for maximum acceptable identifiability. Selection stops after $\\gamma > 15$ or if the maximum number of parameters in a category is selected.', fig.height = 8.5, fig.width = 4.5, echo = F, cache = TRUE, out.width = '0.6\\textwidth'>>=
...
# data prep, select all with coll < 15 plus one, as list
