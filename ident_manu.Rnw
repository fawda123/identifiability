...
% aliased citations
\defcitealias{APHA05}{APHA 2005}
\defcitealias{MurrellIR}{Murrell et al. in revision}
...
\item quantify the identifiability of subsets of the total parameter space based on sensitivity, and
\item provide a set of heuristics for choosing parameters based on sensitivity, identifiability, and parameter categories.
\end{inparaenum}
...
# pairs(caldat[, -1])
temp <- filter(caldat, heurist %in% 'Temperature')
optics <- filter(caldat, heurist %in% 'Optics')
top <- filter(caldat, heurist %in% 'Independent of category')
cattop <- filter(caldat, heurist %in% 'Equally within category')
phyto <- filter(caldat, heurist %in% 'Phytoplankton')

# for individual parameters

ests <- map(cals, 'par') %>%
  map(enframe, name = 'parm', value = 'val') %>% 
  enframe(name = 'heurist') %>%
  unnest %>% 
  mutate(var = 'calib') %>% 
  select(heurist, var, parm, val)

rngs <- get_cmbs(tocal_all, out_var = 'O2', thrsh = 15, coll = T, errs = TRUE, pert = NULL) %>% 
  lapply(function(x){
    x[c('error', 'vals', 'minv', 'maxv')] %>% 
      bind_rows(.id = 'var') %>% 
      gather('parm', 'val', -var)
  }) %>% 
  enframe(name = 'heurist') %>% 
...
prch <- rbind(ests, rngs) %>% 
  spread('var', 'val') %>% 
  mutate(
    pchg = 100 * (calib - vals) / vals,
    pchg = ifelse(pchg > 0, round(pchg, 1), 
      ifelse(pchg < 0, round(pchg, 1), NA)
    ),
    minv = sapply(minv, scinot), 
    maxv = sapply(maxv, scinot), 
    vals = vals,
    calib = sapply(calib, scinot),
    rgrp = ifelse(heurist == 'top', '2', ifelse(heurist == 'cattop', '3', '1')), 
    heurist = factor(heurist, levels = levs, labels = labs), 
    parm = par_txt(parm)
  ) %>% 
  unite('minmax', minv, maxv, sep = '/') %>% 
  select(rgrp, heurist, parm, error, minmax, vals, calib, pchg) %>% 
  arrange(heurist, -error) %>% 
  mutate(
    heurist = as.character(heurist), 
    heurist = ifelse(duplicated(heurist), '', heurist)
    ) 
nc <- prch %>% 
  filter(is.na(pchg))
ncmax <- prch[which.max(prch$pchg), ]
@

Calibration of parameter values for \ac{do} for each of the seven selection heuristics (\cref{tab:heurist1,tab:heurist2,tab:heurist3}) produced model output that varied in goodness of fit and computational effort required to identify optimal values (\cref{tab:calib}).  As expected, the number of required iterations of the optimization algorithm to minimize \ac{rmse} of observed \ac{do} and model output varied in proportion to the number of parameters in a subset.  For example, only \Sexpr{toeng(temp$its)[[1]]} iterations were required to optimize the single parameter selected for the temperature category, whereas \Sexpr{phyto$its} iterations were required for the \Sexpr{toeng(phyto$nprm)[[1]]} parameters selected for phytoplankton (\cref{fig:calex}).  No association was observed between collinearity ($\gamma$) and number of iterations or calibration.  Of the seven parameter subsets, the optimal phytoplankton parameters had the largest reduction in \ac{rmse} values from the initial output (starting \ac{rmse} \Sexpr{strt} mmol O$_2$ m$^{-3}$, reduction to \Sexpr{scinot(phyto$rmse)}).  The identifiable parameters selected equally within categories (third heuristic) and independent of category (second heuristic) produced the second and third greatest reduction in \ac{rmse} (\Sexpr{scinot(cattop$rmse)}, and \Sexpr{scinot(top$rmse)}, respectively), although much less than that observed for phytoplankton parameters. Overall, calibration to most of the parameter subsets had a small effect on \ac{rmse}.  However, the magnitude of parameter changes for calibration varied up to several orders from the initial values (\cref{tab:calibprm}).  The percent change in parameters from the intial values varied from no change (optics parameters \Sexpr{nc$parm[1]} and \Sexpr{nc$parm[2]}) to 900\% (organic matter parameter \Sexpr{ncmax$parm}).  Many calibrated parameter values included those at the lower or upper bounds used for the optimization routine (as for phytoplankton, \cref{fig:calex}).  There was no relationship between magnitude of parameter changes within a subset and overall reduction of \ac{rmse}.

<<>>=
data(area)

# areats
maxdef <- areats[which.max(areats$Original), ]
...
# default august area mean +- se
deftxt <- filter(areatxt, mo %in% 'Aug') %>% 
  .$Original %>% 
  t.test
deftxt <- with(deftxt, c(estimate, diff(conf.int)/2)) %>% 
  round

# mean difference and se from default
levs <- c('umax', 'ZKa', 'astar490', 'KG1', 'Tref')
labs <- paste0(c('umax', 'ZKa', 'astar490', 'KG1', 'Tref(nospA+nospZ)'), '_1')
labs <- par_txt(labs, frm = 'tex')
modiffs <- gather(areatxt, 'var', 'val', -dts, -mo, -Original) %>% 
  group_by(mo, var) %>% 
  summarise(
    se = diff(t.test(val, Original)$conf.int)/2,
    est = mean(val) - mean(Original)
  ) %>% 
  ungroup %>% 
  mutate(
    var = gsub('\\+50%', '', var), 
...
# umax July area mean inrease +- se
umaxtxt <- filter(modiffs, mo %in% 'Jul') %>% 
  filter(grepl('umax', var)) %>% 
  select(est, se) %>% 
  round

# tref July area mean decrease +- se
treftxt <- filter(modiffs, mo %in% 'Jul') %>% 
  filter(grepl('Tref', var)) %>% 
  select(est, se) %>% 
  round

load(file = 'data/sens_ests_cat_all.RData')
senstop <- sens_ests_cat_all[['O2']] %>% 
  group_by(Category) %>% 
  filter(error %in% max(error)) %>% 
  ungroup %>% 
  arrange(error) %>% 
  mutate(
...
% parameter info
<<parmtab, eval = T, cache = F>>=
parmtab(tabsize = 'scriptsize')
@
...
foot.val <- paste0('\\footnotesize *Starting \\ac{rmse} ', strt)
...
<<calibprm, eval = T, cache = F, warning = F>>=
...
totab <- rbind(ests, rngs) %>% 
  spread('var', 'val') %>% 
  mutate(
    pchg = 100 * (calib - vals) / vals,
    pchg = ifelse(pchg > 0, paste0('+', round(pchg, 1)), 
      ifelse(pchg < 0, paste0('-', round(pchg, 1)), 'nc')
    ),
    minv = sapply(minv, scinot), 
    maxv = sapply(maxv, scinot), 
...
cap_val <- 'Results of parameter changes from model calibration to observed \\ac{do} for each of seven parameter subsets selected by sensitivity within each parameter category (first), independent of category (second), and equally within each category (third). The parameters of each subset are from \\cref{tab:heurist1,tab:heurist2,tab:heurist3}.  The L1 values are relative sensitivity of \\ac{do} to a parameter change.  Initial and final parameter values after calibration are shown, including the percent change.  The calibration routine was bounded by minimum and maximum values for each parameter.'
colnms <- c('Parameter', 'L1', 'Min/Max', 'Initial', 'Final', '\\% change')
...
latex(
  ...
  label = 'tab:calibprm',
  rowname = totab$heurist,
  rgroup = names(table(totab$rgrp)),
  n.rgroup = as.numeric(table(totab$rgrp)),
  colheads = colnms
  )
...
\caption{Conceptual representation of biogeochemical components included in FishTank (complete equations in \citealt{Eldridge10}, appendices A-F in \citealt{Lehrter17}). The \acl{cgem} couples FishTank with a hydrodynamic model that includes advection, mixing, and dispersion.}
\label{fig:fishmod}
\end{figure}

% combination example
<<combnex, eval = T, echo = F, fig.cap = 'Examples of unique parameter combinations from different parameter sets and number of selected parameters.  The number of combinations are shown for increasing numbers of selected parameters from the total in the set, where 50 parameter sets are shown each with one through 50 total parameters. Note that the number of unique combinations is shown as the natural-log.', cache = TRUE, fig.height = 5, fig.width = 5, out.width = '0.6\\textwidth'>>=
...
p2 <- ggplot(toplo2, aes(x = N, group = N)) + 
  geom_errorbar(aes(ymin = min, ymax = max), width = 0, colour = lncol) + 
  geom_point(aes(y = ave, shape = 'Average', fill = 'Average'), colour = lncol, size = sz) + 
  geom_point(aes(y = med, shape = 'Median', fill = 'Median'), colour = lncol, size = sz) + 
  scale_shape_manual(name = '', labels = c('Average', 'Median'), values = c(21, 24)) + 
  scale_fill_manual(name = '', labels = c('Average', 'Median'), values = cols[c(5, 3)]) +
...
ggplot(toplo, aes(x = N, y = coll, fill = inc)) + 
  ...
  geom_boxplot(outlier.size = 0.5, size = 0.4) + 
...
p5 <- ggmap(map) + 
  ...
  mytheme %+replace%
  theme(legend.position = 'none') +
  ggtitle(label = NULL, subtitle = '(e) Estimated areal extents on August 28, default extent in grey') + 
